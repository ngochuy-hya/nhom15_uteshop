...
export default function Login() {
  ...
  useEffect(() => {
    if (authService.isAuthenticated()) {
      ...
      const closeButton = document.querySelector('#login [data-bs-dismiss="offcanvas"]');
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Gọi API đăng nhập từ auth service (từ utils/auth.ts)
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          // Không cần reload, chỉ cần trigger state update
