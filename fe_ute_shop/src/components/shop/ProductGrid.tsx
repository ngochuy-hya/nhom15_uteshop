...
export default function ProductGrid({
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (query?.mostViewed) {
                    products = await productsApi.getMostViewedProducts(skeletonCount);
                } else if (query?.tag === "featured") {
                    if (query?.tab === "new") {
                        products = await productsApi.getLatestProducts(skeletonCount);
                    } else if (query?.tab === "best") {
                        products = await productsApi.getBestSellingProducts(skeletonCount);
                    } else if (query?.tab === "deals") {
                        products = await productsApi.getHighestDiscountProducts(skeletonCount);
                    } else {
                        products = await productsApi.getLatestProducts(skeletonCount);
                    }
                } else {
                ...
                if (mounted) {
                    ...
                    setErr(error.message || "Không thể tải sản phẩm");
                }
            } finally {
    ...
    }, [query?.mostViewed, query?.tag, query?.tab, skeletonCount]);
